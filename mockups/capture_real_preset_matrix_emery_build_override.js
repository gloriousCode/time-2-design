...
function renderIndex() {
  ...
  for (const presetKey of Object.keys(PRESETS)) {
    ...
    for (const variant of VARIANTS) {
      ...
      cards.push(`\n<figure><img src="./${fileName}" alt="${preset.label} ${variant.label}" /><figcaption><strong>${preset.label}</strong><br/>${variant.label}</figcaption></figure>`);
    }
  }

  const html = `<!doctype html><html><head><meta charset="utf-8"/><title>Real Matrix (Build Override)</title><style>body{font-family:Arial,sans-serif;margin:16px;background:#f1f1f1;color:#222}.grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(220px,1fr));gap:12px}figure{margin:0;padding:8px;background:#fff;border:1px solid #ddd;border-radius:8px}img{display:block;width:100%;height:auto;background:#ddd;border-radius:6px;image-rendering:pixelated}figcaption{font-size:12px;margin-top:6px;line-height:1.4}</style></head><body><h1>Real Pebble Matrix (Build Override)</h1><div class="grid">${cards.join("\n")}</div></body></html>`;
  fs.writeFileSync(path.join(OUT_DIR, "index_compare.html"), html, "utf8");
}
